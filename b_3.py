...
class HarmonizationModel(nn.Module):
    ...
    def classify(self, z: torch.Tensor) -> torch.Tensor:
        # side by side training to give class signal to encoders
...
def train_harmonization_multi(
    ...
    pair_weighting : "static" uses target_pair_weight, fixed for the whole
                 ...
                 a pair with more residual misalignment right now gets more
                 gradient, full stop. The EMA is always ONE STEP LAGGED: a
                 batch's weights come from the EMA as it stood BEFORE that
                 batch, and only get updated with that batch's results
                 afterward. This is purely a variance-reduction measure --
                 MMD at small batch sizes is a high-variance estimator, and
                 the lag just keeps a single noisy batch from being both
                 the thing optimized and its own weight in the same step.
                 It is NOT preventing anything "gameable": these are
                 detached floats with no gradient path through the
                 weighting itself, so there was never a self-referential-
                 optimization risk to guard against.
                 Still noisy: weighting_ema_beta close to 1 is the main
    ...
    for epoch in range(n_epochs):
        ...
        # freeze at all) -- never let stage-1 recon plateaus trigger an
        # early stop before MMD/clf training has even started
