...
def _oversampling_sampler(num_patients: int, num_samples: int, replacement: bool) -> WeightedRandomSampler:
    """Uniform-weight sampler drawing num_samples indices from num_patients
    each epoch. replacement=True (non-driver cohorts): random-WITH-replacement,
    so a smaller cohort seen multiple times per epoch gets a fresh random draw
    each pass instead of itertools.cycle's fixed, identical, repeated batch
    sequence -- less repetitive input for BatchNorm running stats.
    replacement=False (driver cohort): every patient drawn exactly once per
    epoch, same guarantee plain shuffle=True gives.
    No class-imbalance weighting here (unlike the per-cohort-encoder version)
    -- this script has no aux classifier, so there's nothing to balance for;
    weights are uniform and only replacement/num_samples do any work.
    """
    weights = np.ones(num_patients)
...
def _zip_cycled(loaders: list[DataLoader]):
    ...
    Only used for VAL loaders now -- train loaders are pre-sized to match
    via _oversampling_sampler, so this degrades to a plain zip() for them
    and the repeated-sequence issue itertools.cycle has doesn't arise
    there. It's still fine for val: no training happens during val, so a
    repeated batch order across laps doesn't bias anything the way it
    would for BatchNorm stats / gradient diversity during training.
    """
...
def train_harmonization(
    ...
    patience : early-stopping patience, counted in epochs with no val_loss
                         improvement. Only checked once stage 2 has started
                         (epoch >= decoder_freeze_epoch), or from epoch 0 if
                         decoder_freeze_epoch is None -- otherwise a stage-1
                         reconstruction-only plateau (very plausible once
                         the autoencoder converges and is just oscillating
                         on floating-point noise) could trigger early
                         stopping before mmd/stage-2 training ever starts.
    """
    ...
    # instead of relying on itertools.cycle to repeat a fixed batch sequence.
    driver_name = max(cohort_names, key=lambda n: len(train_cohorts[n]))
    driver_size = len(train_cohorts[driver_name])
    print(f"driver cohort: {driver_name!r} ({driver_size} patients) -- others oversampled to match")

    train_loaders = []
    for name in cohort_names:
        patients = train_cohorts[name]
        is_driver = name == driver_name
        num_samples = len(patients) if is_driver else driver_size
        train_loaders.append(DataLoader(
            VolumeDataset(patients), batch_size=batch_size,
            sampler=_oversampling_sampler(
                len(patients), num_samples=num_samples, replacement=not is_driver
            ),
        ))
    val_loaders = [
    ...
    for epoch in range(n_epochs):
        ...
        # patience only governs stage 2 (or the whole run, if there's no
        # freeze at all) -- never let a stage-1 recon plateau trigger an
        # early stop before mmd/stage-2 training has even started
        stage2_active = decoder_freeze_epoch is None or epoch >= decoder_freeze_epoch
        if stage2_active and epochs_since_improvement >= patience:
