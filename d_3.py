...
def _adaptive_pair_weights(
    ...
    # Tiny positivity guard only: unbiased finite-sample MMD estimates can
    # occasionally be slightly negative, which is unsafe under a fractional
    # exponent. The meaningful floor is applied after normalization below.
    eps = 1e-8
    raw = {
    ...
    # Global rescaling does not change pairwise_mmd's weighted average, but
    # normalizing before clamping keeps tiny absolute values from all hitting
    # the floor and losing their relative differences.
...
def train_harmonization_multi(
    ...
    weighting_temperature : exponent applied to each pair's EMA
                 value (ema ** (1/temperature)) before use as a weight.
                 A tiny positive epsilon guards against slightly negative
                 finite-sample MMD estimates before exponentiation. The raw
                 weights are then normalized to mean 1 before clamping.
                 ...
                 adaptive mode.
    weighting_floor, weighting_ceil : clamp bounds applied to the normalized
                 relative pair weights. The floor/ceil no longer operate on
                 raw EMA magnitude; they bound how small or large a pair's
                 relative emphasis can become after temperature scaling.
    """
            ...
            for batch_dict in _cycling_batches(val_loaders):
                ...
                mmd_latent, _ = pairwise_mmd(
                    ...
                    pair_weights=_adaptive_pair_weights(
                        pair_mmd_ema, pair_weighting, weighting_temperature,
                        weighting_floor, weighting_ceil,
                    ),  # frozen for val -- never updated from val batches
                )
                mmd_image, _ = pairwise_mmd(x_hats, fixed_gamma=DOMAIN_SHIFT_GAMMA)
